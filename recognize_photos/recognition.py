...
def get_result(buf, ANSWER_KEY):
    ...
    #check = np.zeros(thresh.shape, dtype="uint8")
    #sizes = [5, 4, 5, 3, 5]
    #sizes = [5] * len(ANSWER_KEY)
    ...
    for (q, size) in enumerate(sizes):
        ...
        # bubbled answer
        cnts = contours.sort_contours(questionCnts[lastUnusedQuestion:lastUnusedQuestion + size])[0]
